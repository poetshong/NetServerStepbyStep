...
void Acceptor::accept()
{
    ...
    printf("Acceptor::accept() client fd: %d\n", clientfd);
    if (clientfd > 0)
    {
        if (newConnectionCallback_)
        {
            printf("Acceptor::accept(): Run newConnectionCallback\n");
...
struct sockaddr_in Acceptor::getLocalAddress(int sockfd)
{
    return acceptSocket_.getLocalAddr(sockfd);
}